...
public record Rover(FacingDirection facingDirection, Position position) {
    public Rover move(String command) {
        ...
        if (Objects.equals(command, "f")) {
            return new Rover(facingDirection,
                    new Position(this.position().x() + this.facingDirection().getForwardMoveX(), this.position().y() + this.facingDirection().getForwardMoveY()));
